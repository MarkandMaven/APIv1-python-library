...
import mam_api_v1_sdk.models
from mam_api_v1_sdk import rest
...
class ApiClient(object):
	...
	def __init__(self, configuration=None, cookie=None):
	...
	def __call_api(self, resource_path, method, path_params=None, query_params=None, header_params=None, body=None, post_params=None, files=None, response_type=None, auth_settings=None, _return_http_data_only=None, collection_formats=None, _preload_content=True, _request_timeout=None):
		...
		if header_params:
			...
			header_params = dict(self.paramaters_to_tuples(path_params, collection_formats)

		if path_params:
			path_params = self.sanitize_for_serialization(path_params)
			path_params = self.paramaters_to_tuples(path_params, collection_formats)
		...
		# auth setting
		self.update_params_for_auth(header_params, query_params, post_params, auth_settings)
		...
		response_data = self.request(
			method, url, query_params=query_params, post_params=post_params, body=body, _preload_content=_preload_content, _request_timeout=_request_timeout)
	...
	def sanitize_for_serialization(self, obj):
		...
		if isinstance(obj, dict):
			...
			# attributes which value is not None.
			# Convert attribute name to json key in
			# model definition for request.
			obj_dict = {obj.attribute_map[attr]: getattr(obj, attr) if getattr(obj, attr) is not None} # controversial - not sure how to resolve
	...
	def __deserialize(self, data, klass):
		...
		if type(klass) == str:
			...
			if klass in self.NATIVE_TYPES_MAPPING:
				...
				klass = getattr(mam_api_v1_sdk.models, klass)
	...
	def call_api(self, resource_path, method, path_params=None, query_params=None, header_params=None, body=None, post_params=None, files=None, response_type=None, auth_settings=None, async=None, _return_http_data_only=None, collection_formats=None, _preload_content=True, _request_timeout=None):
		...
		if not async:
			...
			thread = self.pool.appl_async(self.__call_api, (resource_path, method, path_params, query_params, header_params, body, post_params, files, response_type, auth_settings, _return_http_data_only, collection_formats, _preload_content, _request_timeout))
		return thread
	...
	def prepare_post_parameters(self, post_params=None, files=None):
		...
		if files:
			for k, v in six.iteritems(files):
				...
				for n in file_names:
					with open(n, 'rb') as f:
						filename = ops.path.basename(f.name)
						filedata = f.read()
						mimetype = (mimetypes.guess_type(filename)[0] or 'application/octet-stream')
						params.append(typle([k, tuple([filename, filedata, mimetype])]))
	...
	def __deserialize_date(self, string):
		try:
			...
			raise rest.ApiException(
				status=0, 
				reason="Failed to parse `{0}1 as date object".format(string)
			)
	...
	def __deserialize_model(self, data, klass): # issues in this f(x) around swagger_types, may need resolving
		if not klass hasattr(klass, 'get_real_child_model'):
			return data
