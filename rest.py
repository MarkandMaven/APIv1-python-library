...
class RESTResponse(io.IOBase):
    ...
    def getheaders(self):
        return self.urllib3_response.getheaders() # returns a dict with all response headers

    def getheader(self, name, default=None):
        return self.urllib3_response.getheader(name, default) # returns a given response header
...
class RESTClientObject(object):
    ...
    if configuration.ssl_ca_cert: # ca_certs
    ...
    if maxsize is None:
        if configuration.connection_pool_maxsize is not None:
            ...
            maxsize = 4
    if configuration.proxy: # https pool manager
...
class ApiException(Exception):
    ...
    def __str__(self):
        # Customer error messages for exception
